---
name: react
description: 'LobeHub React component conventions. Use when editing jsx UI, choosing base-ui vs @lobehub/ui vs antd, styling with antd-style, routing, desktop variants, layouts, or component state.'
...
| -------------------------- | ---------------------------------- |
| `desktopRouter.config.jsx` | `desktopRouter.config.desktop.jsx` |
| `componentMap.ts`          | `componentMap.desktop.ts`          |

**After editing any `.ts`/`.jsx`:** glob for `<filename>.desktop.{ts,jsx}` in the same directory. If found, apply the equivalent sync-import change.
...
| React Router DOM   | Main SPA   | `desktopRouter.config.jsx` + `.desktop.jsx` (pair) |
...
| Editing only `desktopRouter.config.jsx`                            | Must edit both `.jsx` and `.desktop.jsx`                                    |
